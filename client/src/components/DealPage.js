...
const DealPage = () => {
  ...
  const onEditDeal = (updatedDeal) => {
    // Update deal with new data (including rollback cases)
    setDeal(updatedDeal);
  };

  const onEditNote = (updatedNote) => {
    // Update note in the deal's notes array
    setDeal({
      ...deal,
      notes: deal.notes.map(n =>
  ...
  const onDeleteNote = (noteData) => {
    if (noteData._shouldRemove) {
      // Remove note from UI
      setDeal({
        ...deal,
        notes: deal.notes.filter(n => n.id !== noteData.id)
      });
    } else {
      // Restore note (rollback)
      setDeal({
        ...deal,
        notes: [...deal.notes, noteData]
      });
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          <>
            {/* Header Skeleton */}
            <SkeletonLoader type="deal-header" />
        ...
        {!loading && (
          <>
            ...
            {!showEdit && (
              <div className="mb-8">
                <EditDealForm id={deal.id} getDeal={getDeal} deal={deal} onEditDeal={onEditDeal} />
              </div>
            )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {deal.notes.map((note, index) => (
                      <div key={note.id || index}>
                        <NotesCard note={note} amount={deal.amount} getDeal={getDeal} onEditNote={onEditNote} onDeleteNote={onDeleteNote} />
                      </div>
                    ))}
                  </div>
                ) : (
